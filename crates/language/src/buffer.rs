...
use crate::{
    ...
    row_chunk::RowChunks,
    syntax_map::{
        SyntaxLayer, SyntaxMap, SyntaxMapCapture, SyntaxMapCaptures,
...
impl BufferSnapshot {
    ...
    fn suggest_autoindents(
        ...
        let mut prev_row = row_range.start;
        while prev_row > 0 {
            prev_row -= 1;
            if !self.is_line_blank(prev_row) {
                break;
            }
        }

        let language_at = |row: u32| {
            self.language_at(Point::new(row, self.indent_size_for_line(row).len))
        };

        let mut prev_language = language_at(prev_row);

        let mut rows = Vec::new();
        self.for_each_line(
            Point::new(prev_row, 0)..Point::new(row_range.end, 0),
            |row, line| {
                if row == prev_row || row >= row_range.start {
        ...
        Some(row_range.map(move |row| {
            ...
            let this_language = language_at(row);

            let indent = prev_language
                .map(|language| language.config().auto_indent_strings.clone());
    ...
    pub fn resolve_file_path(&self, cx: &App) -> Option<String> {
        self.file().map(|file| file.full_path(cx).to_string_lossy().into_owned())
    }
